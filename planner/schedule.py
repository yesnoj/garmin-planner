...
def cmd_schedule_workouts(args):
    ...
    with GarminClient(args.garmin_id, args.garmin_password) as client:
        ...
        if response[0] not in [200, 204]:
            logging.warn(str(response[0]) + ': ' + str(response[1]))
        ...
        for k, v in training_sessions.items():
            res = re.search(r'\sW(\d\d)S(\d\d)\s', v)
            week = int(res.group(1))
            session = int(res.group(2))
            plan_week = training_plan.get(week, {})
            plan_day = plan_week.get(session, [])
            plan_day.append(k)
            plan_week[session] = plan_day
            training_plan[week] = plan_week

        training_plan = dict(sorted(training_plan.items()))
        train_weeks = list(training_plan.keys())[-1]
        race_day = datetime.datetime.strptime(args.race_day, '%Y-%m-%d')
        first_monday = race_day + datetime.timedelta(days=-race_day.weekday(), weeks=-(train_weeks-1))

        for week_nb, plan in training_plan.items():
        ...
        for k, v in scheduled_plan.items():
            for workout in v:
                logging.info(f'scheduling workout {workout} on {k}')
